  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white p-6 rounded shadow-lg">
        ...
        <img src={device.image} alt={device.name} className="w-32 h-32 mx-auto my-4" />
        <button className="bg-red-500 text-white p-2 rounded" onClick={() => setSelectedDevice(null)}>Close</button>
      </div>
    </div>
  );
}
